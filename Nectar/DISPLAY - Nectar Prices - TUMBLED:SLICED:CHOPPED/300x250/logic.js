...
function setupDom() {
  ...
  app.dom.nectarPricesContainer = document.getElementById(
    "nectarprices-container"
  );
...
function setDynamicContent() {
  ...
  for (var i = 0; i < variantImages.length; i++) {
    ...
    if (variantImages[i] === "product") {
      ...
      createVariantImages.setAttribute(
        "src",
        path + "product-" + productSKU + ".png"
      );
      createVariantImages.setAttribute(
        "srcset",
        path + "product-" + productSKU + ".png 1x"
      );
    } else {
      createVariantImages.setAttribute(
        "src",
        path + variant + "-" + variantImages[i] + ".png"
      );
      createVariantImages.setAttribute(
        "srcset",
        path +
...
var formatCurrency = function(cost) {
  var amount = parseFloat(cost.replace("£", ""))
    .toFixed(2)
    .split(".");
  var formattedPrice;
